...
public:
  ...
  void allocate(int length) {
    	...
    	glyphfeatures.resize(length);
    	spellfeatures.resize(length);
  }
  ...
  void copyValuesFrom(const Instance& anInstance) {
    	...
    	for (int i = 0; i < anInstance.size(); i++) {
      		...
      		glyphfeatures[i] = anInstance.glyphfeatures[i];
      		spellfeatures[i] = anInstance.spellfeatures[i];
    	}
...
public:
  ...
  vector<string> glyphfeatures;
  vector<string> spellfeatures;
};
