...
public:
  ...
  Instance* nextInstance(size_t &posi) {
    	Instance *pInstance = m_jstReader->getNext(posi);
  ...
  Instance* readInstanceByPosi(size_t &posi){
	Instance *pInstance = m_jstReader->getNextByPosi(posi);
	if (!pInstance || pInstance->labels.empty())
              return 0;
    return pInstance;
  ...
  void readInstances(const string& m_strInFile, vector<Instance>& vecInstances, int maxInstance = -1) {
    ...
    void readInstancesPosi(const string& m_strInFile, vector<size_t>& vecInstancesPosi, int maxInstance =-1){
        vecInstancesPosi.clear();
	    initInputFile(m_strInFile.c_str());
	    size_t _inf_current_posi;
	    size_t this_posi;
        Instance *pInstance = nextInstance(_inf_current_posi);
	
        int numInstance = 0;
        while (pInstance) {
		    if (pInstance->size() < 300) {
			    this_posi = _inf_current_posi;
			    vecInstancesPosi.push_back(this_posi);
        	    numInstance++;
    	    ...
    	    pInstance = nextInstance(_inf_current_posi);
